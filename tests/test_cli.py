...
def test_service_default_payload_omits_aggregate_for_single_provider(monkeypatch, tmp_path: Path):
    codex_home = tmp_path / "codex"
    ...
    # Only one provider has data, so the aggregate card is redundant and omitted.
    assert [provider["provider"] for provider in bundle.payload["providers"]] == ["codex"]
...
def test_day_command_reports_single_day_breakdown(tmp_path: Path):
    claude_config = tmp_path / "claude"
    write_jsonl_file(
        claude_config / "projects" / "demo" / "session.jsonl",
        [
            json.dumps(
                {
    ...
    assert "1.5K" in result.output  # 1000 + 500 tokens rendered compactly


def test_day_command_reports_no_usage_for_empty_date(tmp_path: Path):
    claude_config = tmp_path / "claude"
    write_jsonl_file(
        claude_config / "projects" / "demo" / "session.jsonl",
        [
...
def test_hourly_breakdown_buckets_tokens_by_local_hour(tmp_path: Path):
    claude_config = tmp_path / "claude"
    output_path = tmp_path / "out.json"
    write_jsonl_file(
        claude_config / "projects" / "demo" / "session.jsonl",
        [
            json.dumps({
                ...
                "requestId": "req-1",
                "message": {"id": "msg-1", "model": "claude-opus-4-8", "usage": {"input_tokens": 1000, "output_tokens": 500}},
            }),
            json.dumps({
                "timestamp": "2026-03-15T14:30:00",
                "requestId": "req-2",
                "message": {"id": "msg-2", "model": "claude-opus-4-8", "usage": {"input_tokens": 100, "output_tokens": 50}},
            }),
            json.dumps({
                "timestamp": "2026-03-15T21:00:00",
                "requestId": "req-3",
                "message": {"id": "msg-3", "model": "claude-opus-4-8", "usage": {"input_tokens": 10, "output_tokens": 5}},
            }),
        ],
    )
    ...
    assert result.exit_code == 0, result.output
    payload = json.loads(output_path.read_text(encoding="utf-8"))
    day = next(r for r in payload["providers"][0]["daily"] if r["date"] == "2026-03-15")
    hourly = day["hourly"]
    assert len(hourly) == 24
    assert hourly[14] == 1650  # (1000+500) + (100+50) in the 14:00 hour
    assert hourly[21] == 15  # 10+5 in the 21:00 hour
    assert sum(hourly) == day["total"]
