  ...
  <head>
    ...
    <style>
      ...
      * {{ box-sizing: border-box; }}
      ...
      .day-table th:first-child, .day-table td:first-child {{
        text-align: left;
      }}
      .day-empty {{
        color: var(--muted);
  ...
  <body>
      ...
      <div class="page-header">
        ...
        <div class="page-actions">
          <div class="toolbar" id="toolbar">
            <input type="date" id="day-picker" class="day-picker" aria-label="Jump to a specific day" />
    ...
    <script>
      ...
      function renderDayDetail(dateStr) {{
        ...
        const providersWithData = order.filter((providerId) => {{
          if (providerId === "all") return false;
          const provider = providerMap.get(providerId);
          const row = provider && (provider.daily || []).find((item) => item.date === dateStr);
          return !!(row && row.total > 0);
        }});
        const showAggregate = providersWithData.length > 1;
        const blocks = [];
        for (const providerId of order) {{
          if (providerId === "all" && !showAggregate) continue;
          const provider = providerMap.get(providerId);
          if (!provider) continue;
          const row = (provider.daily || []).find((item) => item.date === dateStr);
          if (!row || row.total <= 0) continue;
          const rows = (row.breakdown || []).map((item) =>
            `<tr><td>${{item.name}}</td><td>${{formatTokenTotal(item.tokens.input)}}</td><td>${{formatTokenTotal(item.tokens.output)}}</td><td>${{formatTokenTotal(item.tokens.total)}}</td><td>${{typeof item.costUsd === "number" && item.costUsd > 0 ? formatCost(item.costUsd) : "-"}}</td></tr>`
          ).join("");
          const table = rows
            ? `<table class="day-table"><thead><tr><th>Model</th><th>Input</th><th>Output</th><th>Total</th><th>Cost</th></tr></thead><tbody>${{rows}}</tbody></table>`
            : `<div class="day-empty">No model breakdown</div>`;
          const costText = typeof row.costUsd === "number" && row.costUsd > 0
            ? `  ·  est. ${{formatCost(row.costUsd)}}`
            : "";
          const hourly = row.hourly || [];
          const hourMax = hourly.reduce((max, value) => Math.max(max, value), 0);
          const hourlyHtml = hourMax > 0
            ? `<div class="day-hours">` + hourly.map((value, hour) => {{
                const pct = Math.max(Math.round((value / hourMax) * 100), value > 0 ? 6 : 0);
                const label = String(hour).padStart(2, "0");
                const bar = value > 0
                  ? `<div class="day-hour-bar" style="height:${{pct}}%;background:hsl(${{hour * 15}}, 72%, 55%);"></div>`
                  : "";
                return `<div class="day-hour" title="${{label}}:00 — ${{formatTokenTotal(value)}} tokens"><div class="day-hour-track">${{bar}}</div><div class="day-hour-label">${{hour}}</div></div>`;
              }}).join("") + `</div><div class="day-hours-caption">By hour (0–23, local time) · hover a bar</div>`
            : `<div class="day-empty">Hourly data not available for this day.</div>`;
          blocks.push(`
            <div class="day-provider">
              <div class="day-provider-name">${{provider.id === "all" ? "Aggregate" : "Provider"}} · ${{provider.title}}</div>
              <div class="day-provider-totals">total ${{formatTokenTotal(row.total)}} tokens${{costText}} — in ${{formatTokenTotal(row.input)}} · out ${{formatTokenTotal(row.output)}} · cache-read ${{formatTokenTotal(row.cache.input)}} · cache-write ${{formatTokenTotal(row.cache.output)}}</div>
              ${{table}}
              ${{hourlyHtml}}
            </div>
          `);
        ...
        const body = blocks.length
          ? blocks.join("")
          : `<div class="day-empty">No usage recorded on this date.</div>`;
        dayDetail.innerHTML = `
          <div class="day-detail-head">
            <div class="day-detail-title">${{formatTooltipDate(dateStr)}}</div>
            <button type="button" class="day-detail-close" aria-label="Close">×</button>
          </div>
          ${{body}}
        `;
        const closeButton = dayDetail.querySelector(".day-detail-close");
        if (closeButton) {{
          closeButton.addEventListener("click", () => {{ dayDetail.hidden = true; }});
        }}
      ...
      if (dayPicker) {{
        dayPicker.min = payload.start;
        dayPicker.max = payload.end;
        dayPicker.addEventListener("change", (event) => renderDayDetail(event.target.value));
      }}
