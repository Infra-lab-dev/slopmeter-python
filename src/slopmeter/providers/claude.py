    ...
    for file_path in get_claude_stats_cache_files():
        ...
        for row in stats_cache.get("dailyModelTokens", []) or []:
            ...
            for raw_model_name, total_tokens in tokens_by_model.items():
                ...
                # Stats-cache rows are day-granular only (no real time-of-day),
                # so pass a date to avoid inventing a fake midnight hour bucket.
                add_daily_token_totals(totals, timestamp.date(), token_totals, model_name)
