...
from .render import (
    ...
    render_usage_heatmaps_png,
    render_usage_heatmaps_svg,
)
...
def get_default_service_provider_ids(summary_lookup: dict[UsageProviderId, UsageSummary]) -> list[UsageProviderId]:
    ...
    real_providers = [provider for provider in summary_lookup if provider != "all"]
    # Only show the aggregate card when more than one provider has data; with a
    # single provider the aggregate just duplicates that provider's card.
    if "all" in summary_lookup and len(real_providers) > 1:
        selected.append("all")
...
def analyze_usage(
    ...
    *,
    ...
    with stdout.status("Analyzing usage data..."):
        start, end = window if window is not None else get_date_window()
...
def get_single_day_window(target: datetime) -> tuple[datetime, datetime]:
    start = target.replace(hour=0, minute=0, second=0, microsecond=0)
    end = target.replace(hour=23, minute=59, second=59, microsecond=999000)
    return start, end


def render_day_report(bundle: AnalysisBundle, target: datetime) -> None:
    target_day = target.date()
    stdout.print(f"\n[bold]📅 {format_local_date(target)} usage[/bold]\n")

    any_data = False
    for summary in bundle.export_providers:
        row = next((item for item in summary.daily if item.date == target_day), None)
        if row is None or row.total <= 0:
            continue
        any_data = True

        theme_key = summary.provider if summary.provider != "all" else "all"
        title = (
            get_merged_provider_title(bundle.aggregate_result.rows_by_provider)
            if summary.provider == "all"
            else HEATMAP_THEMES[theme_key].title
        )
        cost = compute_daily_cost(row)

        stdout.print(f"[bold]{title}[/bold]")
        total_line = f"  total {format_token_total(row.total)} tokens"
        if cost > 0:
            total_line += f"  ·  est. {format_cost(cost)}"
        stdout.print(total_line)
        stdout.print(
            f"  input(+cache) {format_token_total(row.input)}   "
            f"output {format_token_total(row.output)}   "
            f"cache-read {format_token_total(row.cache.input)}   "
            f"cache-write {format_token_total(row.cache.output)}"
        )
        ...
        if row.breakdown:
            ...
            for item in row.breakdown:
                item_cost = compute_model_usage_cost(item)
                table.add_row(
                    item.name,
                    format_token_total(item.tokens.input),
                    format_token_total(item.tokens.output),
                    format_token_total(item.tokens.total),
                    format_cost(item_cost) if item_cost > 0 else "-",
                )
            stdout.print(table)

        hourly = row.hourly or []
        if any(hourly):
            ramp = " ▁▂▃▄▅▆▇█"
            hour_max = max(hourly)
            spark = "".join(
                ramp[max(1, round(value / hour_max * 8))] if value > 0 else " "
                for value in hourly
            )
            stdout.print(f"  by hour (0–23): {spark}")
            top_hours = sorted(
                (hour for hour in range(24) if hourly[hour] > 0),
                key=lambda hour: hourly[hour],
                reverse=True,
            )[:3]
            busiest = " · ".join(
                f"{hour:02d}:00 ({format_token_total(hourly[hour])})" for hour in top_hours
            )
            stdout.print(f"  busiest: {busiest}")
        elif row.total > 0:
            stdout.print("  by hour: (no timestamped detail for this day)")
        stdout.print("")

    if not any_data:
        stdout.print("[yellow]No usage recorded on this date for the selected providers.[/yellow]")


def run_day(values: CliArgValues, target: datetime) -> None:
    window = get_single_day_window(target)
    try:
        bundle = analyze_usage(values, selection_mode="export", window=window)
    except ValueError:
        stdout.print(f"\n[bold]📅 {format_local_date(target)} usage[/bold]\n")
        stdout.print("[yellow]No usage recorded on this date for the selected providers.[/yellow]")
        return
    render_day_report(bundle, target)


@app.command("day")
def day_command(
    date: str | None = typer.Argument(None, help="Target date YYYY-MM-DD (default: today)"),
    providers: list[str] | None = typer.Option(None, "--provider", "-p"),
    all: bool = typer.Option(False, "--all"),
    amp: bool = typer.Option(False, "--amp"),
    claude: bool = typer.Option(False, "--claude"),
